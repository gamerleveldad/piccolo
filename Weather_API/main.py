...
def calculate_heat_misery(wbgt_c: float | None) -> int:
    ...
    if wbgt_f is None or wbgt_f <= 70:
        return 0
    if wbgt_f >= 90:
        return 10
...
def calculate_humidity_misery(vpd_kpa: float | None) -> int:
    if vpd_kpa is None or vpd_kpa >= 1.5:
        return 0
    if vpd_kpa <= 0.5:
        return 10
...
def calculate_density_altitude(
    temp_c: float | None, dewpoint_c: float | None, pressure_mb: float | None
) -> int | None:
...
def calculate_evaporation_gallons(
...
async def get_daily_forecast() -> List[Dict[str, Any]]:
    ...
    now_date_str = datetime.now(local_tz).strftime(
        "%Y-%m-%d"
    ...
    try:
        ...
        for table in daily_tables:
            for record in table.records:
                rec_time = record.get_time().astimezone(local_tz)
                date_key = rec_time.strftime("%Y-%m-%d")
                # PREVENT SHIFT: Skip yesterday's forecast entirely
                if date_key < now_date_str:
                    continue
                day_name = rec_time.strftime("%a")

                hourly_data = hourly_aggregates.get(
                    date_key, {"precip_accum_mm": 0.0, "max_wind_ms": 0.0}
                )
                ...
                results.append(
                    {
                        ...
                        "precip_probability": record.values.get(
...
async def get_current_weather() -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "sea_level_pressure_inhg": hpa_to_inhg(
                raw_data.get("calculated_sea_level_pressure")
            ),
            ...
            "lightning_strike_last_distance": raw_data.get(
...
async def get_hourly_forecast(
    hours: int = 48, start_time: Optional[str] = None, end_time: Optional[str] = None
) -> List[Dict[str, Any]]:
    now = datetime.now(timezone.utc)
    ...
    if start_time:
        try:
            ...
            raise HTTPException(
                status_code=400, detail="Invalid start_time format ISO 8601"
            )
    else:
    ...
    if end_time:
        try:
            ...
            raise HTTPException(
                status_code=400, detail="Invalid end_time format ISO 8601"
            )
    else:
                ...
                forecast_data.append(
                    {
                        ...
                        "wind_direction": record.values.get(
                            "wind_direction_cardinal", ""
                        ),
                        "sea_level_pressure_inhg": hpa_to_inhg(
                            record.values.get("sea_level_pressure")
                        ),
                        "sea_level_pressure_mbar": record.values.get(
                            "sea_level_pressure"
                        ),
...
async def get_forecast_accuracy() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        for table in tables:
            for record in table.records:
                ...
                if obs_temp is not None and fcst_temp is not None:
                    lead_metrics[lead_day]["temp_diffs"].append(
                        abs(c_to_f(obs_temp) - c_to_f(fcst_temp))
                    )

                if obs_feels is not None and fcst_feels is not None:
                    lead_metrics[lead_day]["feels_diffs"].append(
                        abs(c_to_f(obs_feels) - c_to_f(fcst_feels))
                    )
        ...
        for lead_day in range(4):
            ...
            temp_mae = (
                round(sum(data["temp_diffs"]) / len(data["temp_diffs"]), 1)
                if data["temp_diffs"]
                else 0.0
            )
            feels_mae = (
                round(sum(data["feels_diffs"]) / len(data["feels_diffs"]), 1)
                if data["feels_diffs"]
                else 0.0
            )
        ...
        for table in tables:
            for record in table.records:
                ...
                return {
                    ...
                    "sea_level_pressure_inhg": hpa_to_inhg(
                        record.values.get("calculated_sea_level_pressure")
                    ),
                    "wind_avg_mph": ms_to_mph(record.values.get("wind_avg")),
                    "wind_gust_mph": ms_to_mph(record.values.get("wind_gust")),
                    "wind_direction": record.values.get("wind_direction_cardinal", ""),
                    "visibility_index": record.values.get(
                        "calculated_visibility_index"
                    ),
                }
        return {}
    except Exception as e:
    ...
    try:
        ...
        for table in tables:
            for record in table.records:
                ...
                windows_to_update = []
                if delta_hours <= 24:
                    windows_to_update.extend([1, 2, 3])
                elif delta_hours <= 48:
                    windows_to_update.extend([2, 3])
                elif delta_hours <= 72:
                    windows_to_update.append(3)

                for w in windows_to_update:
                    if field == "air_temperature":
                        data[w]["temp"].append(val)
                    elif field == "dew_point":
                        data[w]["dew"].append(val)
                    elif field == "wind_avg":
                        data[w]["wind"].append(val)
        ...
        for days in [1, 2, 3]:
            ...
            evap_gallons = calculate_evaporation_gallons(
                avg_temp, avg_dew, avg_wind, days * 24
            )
...
async def get_active_stormcells() -> List[Dict[str, Any]]:
    async with app.state.pg_pool.acquire() as conn:
        ...
        for r in records:
            cone_narrow = (
                json.loads(r["forecast_cone_narrow"])
                if r["forecast_cone_narrow"]
                else None
            )
            cone_wide = (
                json.loads(r["forecast_cone_wide"]) if r["forecast_cone_wide"] else None
            )
            ...
            cells.append(
                {
                    ...
                    "movement": {
                        ...
                        "speed_mph": ms_to_mph(safe_float(r["speed_kts"]) * 1.15078)
                        if r["speed_kts"]
                        else None,
                    },
                    "severity": {
                    ...
                    "forecast_polygons": {"narrow": cone_narrow, "wide": cone_wide},
                    "traits": traits,
                }
            )
        return cells
            ...
            storms.append(
                {
                    ...
                    "wind_radii": json.loads(r["wind_radii"])
                    if r["wind_radii"]
                    else None,
                    "forecast_track": json.loads(r["forecast_track"])
                    if r["forecast_track"]
                    else [],
                    "breakpoint_alerts": json.loads(r["breakpoint_alerts"])
                    if r["breakpoint_alerts"]
                    else [],
                }
            )
        return storms
